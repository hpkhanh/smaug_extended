...
static Network* createNetworkFromProto(const GraphProto& graphProto,
    ...
    for (int i = 0; i < graphProto.nodes_size(); i++) {
        const NodeProto& node = graphProto.nodes(i);
        const std::string& name = node.name();
        LayerConfig * layer_conf = networkConfig->getLayerConfig(name);
        if (layer_conf == nullptr){
            layer_conf = new LayerConfig();
            layer_conf->layerName = name;
            layer_conf->backend = smaug::Cpu;
            layer_conf->numCores = 1;            
        }        
        createAndAddOperator<Backend>(node,
                                      layer_conf,
                                      backEndConfig,
